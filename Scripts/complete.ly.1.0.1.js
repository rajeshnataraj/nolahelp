...
function completely(container, config) {
    ...
    config.hintColor = config.hintColor || '#fff';  //changed from gray to white to hide hint
    config.backgroundColor = config.backgroundColor || '#fff';
    config.dropDownBorderColor = config.dropDownBorderColor || '#aaa';
    config.dropDownZIndex = config.dropDownZIndex || '10000'; // to ensure we are in front of everybody
    ...
    var createDropDownController = function (elem) {
        ...
        var p = {
            ...
            refresh: function (token, array) {
                //elem.style.visibility = 'hidden';  //uncomment to hide drop down when only one match is available.
                ix = 0;
                elem.innerHTML = '';
                ...
                for (var i = 0; i < array.length; i++) {
                   // if (array[i].indexOf(token) !== 0) { continue; }
                    if (array[i].toLowerCase().indexOf(token.toLowerCase()) !== 0) { continue; }
